...
class MeshCommunicator {
	...
	static void Disconnect() {
		...
		try {
            ...
            System.out.println(DBG_TAG + " Disconnected!");
		} catch (IOException e) {
            System.err.println(DBG_TAG + " Disconnect failed: " + e.getMessage());
		}
	...
	static class ReceiveRunnable implements Runnable {
		...
		ReceiveRunnable(Socket server) {
			...
			try {
				...
				System.err.println(DBG_TAG + " ReceiveRunnable failed: " + e.getMessage());
			}
		}

		@Override
		public void run() {
			System.out.println(DBG_TAG + " Receiving started");
			while (!Thread.currentThread().isInterrupted() && isConnected()) {
				...
				try {
					...
					if (readLen > 0) {
						...
						System.out.println(DBG_TAG + " Received " + readLen + " bytes: " + rcvdMsg);

						System.out.println(DBG_TAG + " Data received!");

						sendMyBroadcast(MESH_DATA_RECVD, rcvdMsg);
					}

				} catch (IOException e) {
					System.err.println(DBG_TAG + " Receiving loop stopped: " + e.getMessage());
					Disconnect(); //Gets stuck in a loop if we don't call this on error!
					sendMyBroadcast(MESH_SOCKET_ERR, e.getMessage());
				}
			}
			receiveThreadRunning = false;
			System.out.println(DBG_TAG + " Receiving stopped");
		}
	...
	static class SendRunnable implements Runnable {
		...
		SendRunnable(Socket server) {
			try {
				...
				System.err.println(DBG_TAG + " Start SendRunnable failed: " + e.getMessage());
			}
		}
		...
		@Override
		public void run() {
			System.out.println(DBG_TAG + " Sending started");
			if (this.hasMessage) {
				try {
					//Send the data
					...
					System.err.println(DBG_TAG + " Sending failed: " + e.getMessage());
					Disconnect(); //Gets stuck in a loop if we don't call this on error!
					sendMyBroadcast(MESH_SOCKET_ERR, e.getMessage());
				}
				this.hasMessage = false;
				this.data =  null;
				System.out.println(DBG_TAG + " Command has been sent!");
			}
			System.out.println(DBG_TAG + " Sending stopped");
		}
	}
	...
	static class ConnectRunnable implements Runnable {
		public void run() {
			try {
				...
				System.out.println(DBG_TAG + " C: Connecting...");
				...
				System.out.println(DBG_TAG + " Connected!");
				...
				System.err.println(DBG_TAG + " Connecting failed: " + e.getMessage());
			}
			sendMyBroadcast(MESH_CONNECTED,"");
			System.out.println(DBG_TAG + " Connection thread finished");
		}
	}
	...
	static void sendMyBroadcast(String action, String msgReceived) {
        IntentManager.SINGLETON.broadcast(action, msgReceived);
	}
}